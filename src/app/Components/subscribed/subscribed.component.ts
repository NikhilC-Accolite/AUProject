...
export class SubscribedComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.subscribeService.getSubscribedDetails(this.userId).subscribe((data: any) => {
      data.forEach((item:any) => {
        if (item.catName != null && item.catName!= "DEFAULT" && item.catName!= "All Category"){
          if(item.subs === false){
      ...
      //this.subscribeArray = data;
      console.log(data);
    });
  }

  subscribeVideo(catName:string){
    this.subscribeService.subscribeVideos(catName, this.userId).subscribe((data: any) => {
