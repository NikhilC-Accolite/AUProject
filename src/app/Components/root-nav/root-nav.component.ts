...
export class RootNavComponent implements OnInit {
  ...
  sendCategory(name: string) {
    ...
    // this.router.navigateByUrl([`/video-category/${this.userId}/${this.categoryId}`]);
  }
  ...
  ngOnInit() {
    ...
    this.serviceObj.getSubscribedCategory(this.userId).subscribe((data: any) => {
      data.forEach((item) => {
  ...
  modifySubscription(){
    //update userId
    //this.router.navigateByUrl(`/subscribed/${userId}`);
